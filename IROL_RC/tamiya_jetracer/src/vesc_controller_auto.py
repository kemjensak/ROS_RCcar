...
class RC_driver():
    def __init__(self):
        ...
        try:
            ...
            if(self.errCount > self.max_errCount):
                rospy.logerr("VESC connection error!")
                err;
            # self.motor.__del__()
            self.__init__()
        ...
        self.tacho_jitter_threshold = rospy.get_param('~tacho_jitter_threshold',10)
        self.last_tacho = 0
        self.tacho_err = 0
        self.controlMode = 1
        self.steer_rad = 0.0
    ...
    def joy_cmdCallback(self, data):
        ...
        if 0.1 < data.linear.x < 0.6:
            data.linear.x = 0.6
        self.speed_cmd = data.linear.x * speed_per_ms
        if data.linear.x != 0:
            self.steer_rad = atan(wheelbase / data.linear.x * data.angular.z)
        else:
            self.steer_rad = 0
